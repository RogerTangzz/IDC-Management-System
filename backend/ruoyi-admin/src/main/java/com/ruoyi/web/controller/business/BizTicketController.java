...
public class BizTicketController extends BaseController {
    ...
    @PreAuthorize("@ss.hasPermi('business:ticket:list')")
    @GetMapping("/nearDue")
    public TableDataInfo nearDue(@RequestParam(value = "hours", required = false) Integer hours){
        int h = (hours != null ? hours : (slaProperties!=null ? slaProperties.getWarnBeforeHours() : 2));
        startPage();
        List<BizTicket> list = bizTicketMapper.selectNearDueList(h);
        return getDataTable(list);
    }
