<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :row-key="getRowKey" @row-click="clickRow" ref="roleRef"
      ...
      <el-table-column :label="t('system.user.authRole.serialNumber')" width="55" type="index" align="center">
    ...
    <el-form label-width="100px">
      <div style="text-align: center;margin-left:-120px;margin-top:30px;">
        <el-button type="primary" @click="submitForm()">{{ t('system.user.authRole.submit') }}</el-button>
        <el-button @click="close()">{{ t('system.user.authRole.return') }}</el-button>
      </div>
    </el-form>
  </div>
</template>
...
(() => {
  ...
  if (userId) {
    loading.value = true
    getAuthRole(userId).then(_response => {
      form.value = _response.user
      roles.value = _response.roles
